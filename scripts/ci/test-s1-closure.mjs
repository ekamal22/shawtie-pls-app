...
const expectedBranch = "feat/s1-e2ee-crypto-recovery";
const baseRef = process.env.S1_BASE_REF ?? "origin/main";
...
function step(name, command, args) {
  console.log("S1_CLOSURE_STEP_START " + name);
  const result = spawnSync(command, args, { encoding: "utf8", stdio: "inherit" });
  if (result.error) throw result.error;
  if (result.status !== 0) {
    throw new Error("S1 closure step " + name + " exited with status " + result.status);
  }
  console.log("S1_CLOSURE_STEP_PASS " + name);
}

const npmCli = process.env.npm_execpath;
if (!npmCli) throw new Error("Run this through npm: npm run test:s1:closure");

const branch = run("git", ["rev-parse", "--abbrev-ref", "HEAD"]);
if (branch !== expectedBranch) {
  throw new Error("S1 closure must run on " + expectedBranch + ". Current branch: " + branch);
}
const initialStatus = run("git", ["status", "--short"]);
if (initialStatus) throw new Error("S1 closure requires a clean worktree.\n" + initialStatus);
...
for (const line of commits.split("\n").filter(Boolean)) {
  ...
  if (!subject.includes("[skip ci]")) {
    throw new Error("S1 commit is missing [skip ci]: " + sha + " " + subject);
  }
}
...
if (introducedEmDash) throw new Error("S1 diff introduces a forbidden em dash.");

step("s1-local", process.execPath, [npmCli, "run", "test:s1:local"]);
...
if (finalStatus) throw new Error("S1 automated closure left a dirty worktree.\n" + finalStatus);
