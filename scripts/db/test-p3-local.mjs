...
const containerName = `shawtie-p3-postgres-${process.pid}`;
const user = "shawtie_test";
const password = "shawtie_test";
const database = "shawtie_p3_test";
...
try {
  ...
  runDocker(
    ...
    [
      ...
      containerName,
      "--label",
      "com.shawtie.role=p3-disposable-postgres",
      "-e",
      "POSTGRES_USER=" + user,
      "-e",
  ...
  console.log("P3_LOCAL_POSTGRES_READY port=" + port);
  ...
  if (testResult.status !== 0) {
    throw new Error(
      "P3 PostgreSQL suite exited with status " + testResult.status,
    );
  }

  console.log("P3_LOCAL_POSTGRES_PASS");
} finally {
  if (started) {
    ...
    if (cleanup.error || cleanup.status !== 0) {
      console.error("P3_LOCAL_POSTGRES_CLEANUP_WARNING", {
        containerName,
      });
    }
  }
