...
const containerName = `shawtie-m1-postgres-${process.pid}`;
const user = "shawtie_test";
const password = "shawtie_test";
const database = "shawtie_m1_test";
...
try {
  ...
  runDocker(
    ...
    [
      ...
      containerName,
      "--label",
      "com.shawtie.role=m1-disposable-postgres",
      "-e",
      "POSTGRES_USER=" + user,
      "-e",
  ...
  console.log("M1_LOCAL_POSTGRES_READY port=" + port);
  ...
  if (testResult.status !== 0) {
    throw new Error(
      "M1 PostgreSQL suite exited with status " + testResult.status,
    );
  }

  console.log("M1_LOCAL_POSTGRES_PASS");
} finally {
  if (started) {
    ...
    if (cleanup.error || cleanup.status !== 0) {
      console.error("M1_LOCAL_POSTGRES_CLEANUP_WARNING", {
        containerName,
      });
    }
  }
