...
const containerName = `shawtie-r1-postgres-${process.pid}`;
const user = "shawtie_test";
const password = "shawtie_test";
const database = "shawtie_r1_test";
...
try {
  ...
  runDocker(
    ...
    [
      ...
      containerName,
      "--label",
      "com.shawtie.role=r1-disposable-postgres",
      "-e",
      "POSTGRES_USER=" + user,
      "-e",
  ...
  console.log("R1_LOCAL_POSTGRES_READY port=" + port);

  const npmCli = process.env.npm_execpath;
  if (!npmCli) {
    throw new Error(
      "npm_execpath is unavailable. Run this command through npm: npm run test:r1:local",
    );
  }

  const testResult = run(
    process.execPath,
    [npmCli, "run", "test:r1:postgres"],
    {
      stdio: "inherit",
      env: {
        ...process.env,
        ...
        SHAWTIE_MIGRATION_RESERVATIONS: "0011,0012",
      },
    },
  );
  ...
  if (testResult.status !== 0) {
    throw new Error(
      "R1 PostgreSQL suite exited with status " + testResult.status,
    );
  }

  console.log("R1_LOCAL_POSTGRES_PASS");
} finally {
  if (started) {
    ...
    if (cleanup.error || cleanup.status !== 0) {
      console.error("R1_LOCAL_POSTGRES_CLEANUP_WARNING", {
        containerName,
      });
    }
  }
