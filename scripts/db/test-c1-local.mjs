...
const containerName = "shawtie-c1-postgres-" + process.pid;
const user = "shawtie_test";
const password = "shawtie_test";
const database = "shawtie_c1_test";
...
try {
  ...
  runDocker(
    ...
    [
      ...
      containerName,
      "--label",
      "com.shawtie.role=c1-disposable-postgres",
      "-e",
      "POSTGRES_USER=" + user,
      "-e",
  ...
  if (!npmCli) throw new Error("Run through npm: npm run test:c1:local");

  console.log("C1_LOCAL_POSTGRES_READY port=" + portMatch[1]);
  const result = run(process.execPath, [npmCli, "run", "test:c1:postgres"], {
    stdio: "inherit",
    env: {
      ...process.env,
  ...
  if (result.error) throw result.error;
  if (result.status !== 0) {
    throw new Error("C1 PostgreSQL suite exited with status " + result.status);
  }

  console.log("C1_LOCAL_POSTGRES_PASS");
  console.log("C1_LOCAL_IMPLEMENTATION_PASS");
} finally {
  if (started) {
    const cleanup = run(docker, ["stop", "--time", "2", containerName], { stdio: "ignore" });
    if (cleanup.error || cleanup.status !== 0) {
      console.error("C1_LOCAL_POSTGRES_CLEANUP_WARNING", { containerName });
    }
  }
}
