...
const containerName = `shawtie-m2-postgres-${process.pid}`;
const user = "shawtie_test";
const password = "shawtie_test";
const database = "shawtie_m2_test";
...
try {
  ...
  runDocker(
    ...
    [
      ...
      containerName,
      "--label",
      "com.shawtie.role=m2-disposable-postgres",
      "-e",
      "POSTGRES_USER=" + user,
      "-e",
  ...
  console.log("M2_LOCAL_POSTGRES_READY port=" + port);

  const npmCli = process.env.npm_execpath;
  if (!npmCli) {
    throw new Error(
      "npm_execpath is unavailable. Run this command through npm: npm run test:m2:local",
    );
  }

  const testResult = run(
    process.execPath,
    [npmCli, "run", "test:m2:postgres"],
    {
      stdio: "inherit",
      env: {
        ...process.env,
  ...
  if (testResult.status !== 0) {
    throw new Error(
      "M2 PostgreSQL suite exited with status " + testResult.status,
    );
  }
  ...
  if (started) {
    ...
    if (cleanup.error || cleanup.status !== 0) {
      console.error("M2_LOCAL_POSTGRES_CLEANUP_WARNING", {
        containerName,
      });
    }
  }
