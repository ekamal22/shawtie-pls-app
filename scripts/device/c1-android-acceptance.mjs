...
const WEB_PORT = Number(process.env.C1_DEVICE_WEB_PORT ?? "4173");
const CDP_PORT = Number(process.env.C1_DEVICE_CDP_PORT ?? "9222");
const APP_URL =
  process.env.C1_DEVICE_URL ?? "http://127.0.0.1:" + WEB_PORT + "/";
...
if (cleanupOnly) {
  ...
  console.log("C1_ANDROID_CLEANUP_PASS serial=" + serial);
  process.exit(0);
}
...
const path = join(
  "validation-logs",
  "c1-android-prepare-" + timestamp() + ".json",
);
await writeFile(path, JSON.stringify(evidence, null, 2) + "\n", "utf8");

console.log(
  "C1_ANDROID_PREPARE_PASS serial="
    + serial
    + " model="
...
console.log("C1_ANDROID_EVIDENCE " + path);
console.log(
  "C1_ANDROID_NEXT run the documented foreground/background, offline/reconnect, lifecycle, revocation, update, and namespace-isolation scenarios manually; do not mark the milestone DONE from this preflight alone.",
);
