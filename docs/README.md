...
C1 Voice Calling and C2 Video Calling are also DONE and merged to `main`. UX0 through UX7 are DONE, physically accepted at executable SHA `ca7cd35` with all 22 mandatory UX scenarios passing on the Xiaomi Redmi Note 9S, and fast-forward merged to `main` at merge anchor `9f0bea4`. S1 E2EE and Cryptographic Recovery is IN_PROGRESS on `feat/s1-e2ee-crypto-recovery`. S1-A through S1-I source implementation and S1-J closure tooling are committed at `0e28675`; automated closure, raw plaintext inspections, physical Android acceptance, and final independent security review remain open. Canonical architecture: `architecture/S1_E2EE_CRYPTO_RECOVERY_DESIGN.md`. UX8 follows verified S1 closure, then R2 Public Readiness.
