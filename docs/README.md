...
C1 Voice Calling source implementation is complete on `feat/c1-voice-calling` from `main @ 54b8659a`. C1 remains IN_PROGRESS because automated/local closure has not yet been executed, mandatory physical Android acceptance is still open, and final integrated migration closure waits for M3 migrations 0015/0016 to merge. The branch includes durable call authority, realtime v2 invalidation, dedicated `shawtie.call.v1` signaling, relay-only WebRTC/TURN, keyed Web Push endpoint fingerprints, multi-tab media ownership, lifecycle/revocation handling, PostgreSQL/worker closure runners, a real-Chromium ownership harness, and Android preflight. C1 is voice-only; C2 remains a separate video milestone. The canonical design is `architecture/C1_VOICE_CALLING_DESIGN.md`, the HTTP contract is `api/C1_CALLING_API.md`, the transient signaling protocol is `api/C1_SIGNALING_PROTOCOL.md`, and physical acceptance is `testing/C1_ANDROID_ACCEPTANCE.md`.
