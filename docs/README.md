...
`ROADMAP.md` is the canonical milestone execution sequence and explicitly identifies physical-device requirements. The current verified post-M2 mainline is `main @ 54b8659a101dcaeb6ff1e0b7caee76921c5b9919`. M3 Media and Voice Messages is DONE and fast-forward merged to `main` at `1d3535f1c4d2d16e66c3bfa4c9c8cef42a95822a`: automated closure is green and all 20 mandatory physical Android scenarios passed at final code SHA `ee59850`. C1 Voice Calling has completed source implementation, final integrated automated/local closure and physical acceptance on `feat/c1-voice-calling`. The integrated closure passed at `9b5c255b5e8c60cbe8da4bcd2b6f7596c56687a0` against real migrations 0001 through 0018 with `reserved=0`, including the M3 and C1 PostgreSQL, MinIO, Chromium, repository-health, audit, and git-hygiene gates. Mandatory physical Android acceptance on the Redmi Note 9S then passed 25/25 (`testing/C1_ANDROID_ACCEPTANCE_EVIDENCE.md`, tested SHA `9cbc2f8`, executable code identical to `9b5c255`), so C1 is DONE on `feat/c1-voice-calling` and is not merged to `main`. C2 Video Calling remains a separate later milestone and is not started. Hosted GitHub Actions verification remains separate under V1.
