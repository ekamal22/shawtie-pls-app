...
C1 Voice Calling architecture and source implementation are complete on `feat/c1-voice-calling`, created from `main @ 54b8659a101dcaeb6ff1e0b7caee76921c5b9919`. Automated/local closure has not yet been executed, mandatory physical Android acceptance remains open, and final integrated closure remains blocked until real M3 0015/0016 are merged. The implemented surface includes coarse endpoint failure reporting, keyed push endpoint fingerprints, a focused real-Chromium media-owner harness, and the existing Android preflight in addition to the core call stack. The third pass closes concrete runtime seams: participant rows are the only endpoint authority, timeout work uses independent deadline generation, endpoint-connected converges without expectedVersion, same-device tabs use one generation-fenced media owner, signaling is voice-only with related-address privacy checks, generic call-state push canonicalizes notification state, realtime v2 call hints participate in M2 anti-entropy/barrier repair, shared WebSocket negotiation preserves M2 limits, TURN post-revocation lifetime is explicitly bounded, and internal security/lifecycle terminal causes map to public outcomes. Canonical design: `docs/architecture/C1_VOICE_CALLING_DESIGN.md`; API: `docs/api/C1_CALLING_API.md`; signaling: `docs/api/C1_SIGNALING_PROTOCOL.md`; physical Android procedure: `docs/testing/C1_ANDROID_ACCEPTANCE.md`. ADR-013 adopts dedicated accepted-call signaling and ADR-014 relay-only TURN privacy. C1 owns implemented migrations 0017/0018 while parallel M3 owns implemented but unmerged 0015/0016; isolated C1 validation may reserve 0015/0016, but final integration requires real 0001-0018 with `reserved=0`. C2 remains separate after C1.
...
- C1 Voice Calling: IN_PROGRESS on `feat/c1-voice-calling`; source implementation complete, verification pending. Implemented work includes dedicated `shawtie.call.v1`, negotiated `shawtie.realtime.v2`, participant-role endpoint authority, independent deadline fencing, same-device tab ownership, relay-only TURN, order-independent Web Push reconciliation, keyed endpoint fingerprints, multi-device first-accept-wins, coarse endpoint failure reporting, lifecycle/deletion integration, migrations 0017/0018, a real-Chromium ownership harness, and Android preflight.
...
4. C1 source implementation is complete on `feat/c1-voice-calling`; run the automated/local closure with M3 0015/0016 reserved
5. after automated closure is green, execute mandatory C1 Android acceptance on the physical Redmi path
6. C1 was implemented in parallel with M3 using reservations; final C1 closure waits for real M3 0015/0016 on main
7. keep C2 video separate and blocked until C1 automated/local closure, physical verification, final integrated closure, and merge are complete
