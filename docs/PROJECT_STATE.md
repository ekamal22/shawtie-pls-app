...
C1 Voice Calling architecture, source implementation, and automated/local closure are complete on `feat/c1-voice-calling`, created from `main @ 54b8659a101dcaeb6ff1e0b7caee76921c5b9919`. `npm run test:c1:closure` passed at `439b09f551512ea79a16e8f3d047a32b9a722203`: M3-owned 0015/0016 were reserved without placeholder files, database invariants passed, C1 fast tests passed 20/20, the PostgreSQL/API/worker matrix passed 111/111, real Chromium passed 2/2, full health passed with Domain 63/63, Contracts 40/40, API unit/security 54/54, and Worker 11/11, the high-severity audit reported 0 vulnerabilities, git hygiene and parity passed, and `C1_AUTOMATED_IMPLEMENTATION_PASS reservations=0015,0016` was recorded. Mandatory physical Android acceptance remains open, and final integrated closure remains blocked until real M3 0015/0016 are merged and the canonical 0001-0018 chain passes with `reserved=0`. The implemented surface includes coarse endpoint failure reporting, keyed push endpoint fingerprints, a focused real-Chromium media-owner harness, and the existing Android preflight in addition to the core call stack. The third pass closes concrete runtime seams: participant rows are the only endpoint authority, timeout work uses independent deadline generation, endpoint-connected converges without expectedVersion, same-device tabs use one generation-fenced media owner, signaling is voice-only with related-address privacy checks, generic call-state push canonicalizes notification state, realtime v2 call hints participate in M2 anti-entropy/barrier repair, shared WebSocket negotiation preserves M2 limits, TURN post-revocation lifetime is explicitly bounded, and internal security/lifecycle terminal causes map to public outcomes. Canonical design: `docs/architecture/C1_VOICE_CALLING_DESIGN.md`; API: `docs/api/C1_CALLING_API.md`; signaling: `docs/api/C1_SIGNALING_PROTOCOL.md`; physical Android procedure: `docs/testing/C1_ANDROID_ACCEPTANCE.md`. ADR-013 adopts dedicated accepted-call signaling and ADR-014 relay-only TURN privacy. C1 owns implemented migrations 0017/0018 while parallel M3 owns implemented but unmerged 0015/0016. C2 remains separate after C1.
...
- WebRTC voice and video, with C1 voice and C2 video kept as separate milestones
- baseline relay-first TURN privacy, refined to relay-only for C1 by accepted ADR-014
...
- C1 Voice Calling: IN_PROGRESS on `feat/c1-voice-calling`; source implementation and automated/local closure complete at `439b09f`, with mandatory physical Android acceptance and final M3-integrated `reserved=0` closure pending. Implemented work includes dedicated `shawtie.call.v1`, negotiated `shawtie.realtime.v2`, participant-role endpoint authority, independent deadline fencing, same-device tab ownership, relay-only TURN, order-independent Web Push reconciliation, keyed endpoint fingerprints, multi-device first-accept-wins, coarse endpoint failure reporting, lifecycle/deletion integration, migrations 0017/0018, a real-Chromium ownership harness, and Android preflight.
...
- `feat/c1-voice-calling` -> source implementation and automated/local closure complete at `439b09f`; remains unmerged pending mandatory physical Android acceptance and final integrated migration closure
...
4. preserve the C1 automated/local closure anchor `439b09f` and its green reserved-migration evidence
5. execute mandatory C1 Android acceptance on the physical Redmi path
6. C1 was implemented in parallel with M3 using reservations; final C1 closure waits for real M3 0015/0016 on main
7. keep C2 video separate and blocked until C1 physical verification, final integrated closure, and merge are complete
