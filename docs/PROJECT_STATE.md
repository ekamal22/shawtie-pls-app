...
C1 Voice Calling architecture is complete and the source implementation is complete on `feat/c1-voice-calling`, created from `main @ 54b8659a101dcaeb6ff1e0b7caee76921c5b9919`. Automated closure has not yet been executed in this environment, mandatory physical Android acceptance remains open, and final integrated closure remains blocked until real M3 0015/0016 are merged. The third pass closes concrete runtime seams: participant rows are the only endpoint authority, timeout work uses independent deadline generation, endpoint-connected converges without expectedVersion, same-device tabs use one generation-fenced media owner, signaling is voice-only with related-address privacy checks, generic call-state push canonicalizes notification state, realtime v2 call hints participate in M2 anti-entropy/barrier repair, shared WebSocket negotiation preserves M2 limits, TURN post-revocation lifetime is explicitly bounded, and internal security/lifecycle terminal causes map to public outcomes. Canonical design: `docs/architecture/C1_VOICE_CALLING_DESIGN.md`; API: `docs/api/C1_CALLING_API.md`; signaling: `docs/api/C1_SIGNALING_PROTOCOL.md`; physical Android procedure: `docs/testing/C1_ANDROID_ACCEPTANCE.md`. ADR-013 adopts dedicated accepted-call signaling and ADR-014 relay-only TURN privacy. C1 owns planned migrations 0017/0018 while parallel M3 owns planned 0015/0016; isolated C1 validation may reserve 0015/0016, but final integration requires real 0001-0018 with `reserved=0`. C2 remains separate after C1.
...
- C1 Voice Calling: IN_PROGRESS on `feat/c1-voice-calling`; source implementation complete, verification pending. Dedicated `shawtie.call.v1`, negotiated `shawtie.realtime.v2`, participant-role endpoint authority, independent deadline fencing, same-device tab ownership, relay-only TURN, order-independent Web Push reconciliation, multi-device first-accept-wins, lifecycle/deletion integration, migrations 0017/0018, and physical Android acceptance are fully designed.
...
6. C1 may implement in parallel with M3 using reservations, but final C1 closure waits for real M3 0015/0016 on main
7. keep C2 video separate and blocked until C1 is implemented, physically verified, and merged
