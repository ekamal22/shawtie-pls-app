...
- C1 Voice Calling: DESIGN COMPLETE, THIRD-PASS HARDENED on `feat/c1-voice-calling`; implementation not started. Dedicated `shawtie.call.v1`, negotiated `shawtie.realtime.v2`, participant-role endpoint authority, independent deadline fencing, same-device tab ownership, relay-only TURN, order-independent Web Push reconciliation, multi-device first-accept-wins, lifecycle/deletion integration, migrations 0017/0018, and physical Android acceptance are fully designed.
...
4. C1 third-pass design is complete on `feat/c1-voice-calling`; implementation may begin from current verified M2 mainline while preserving parallel M3 migration ownership
5. implement C1 against the concrete module map and shared-WebSocket seam in `C1_VOICE_CALLING_DESIGN.md`
