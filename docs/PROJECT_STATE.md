...
- M2 Realtime and Offline Reliability: IN_PROGRESS. Architecture and protocol design are complete and second-pass hardened on `feat/m2-realtime-offline`; runtime implementation has not started. The hardening pass adds a race-free live barrier, immutable socket scope identity, old-connection callback rejection, LISTEN-reset resynchronization, visible anti-entropy, multi-tab claim fencing, pre-S1 cold-start locking, storage-failure semantics, exact R1 offline whitelist, and committed-NOTIFY-before-outbox-ack behavior. M2 preserves HTTP/PostgreSQL authority, M1 server/change sequence separation, partnership-scoped IndexedDB isolation, lifecycle-aware replay, and physical Android closure.
