...
- UX0 Romantic Experience Specification: IN_PROGRESS. Claude's romantic UX concept is accepted as the design direction and is being converted into the canonical implementation-ready specification in `docs/design/ROMANTIC_UX_DIRECTION.md`.
- UX1 through UX7: PLANNED product-experience implementation after UX0, with presentation-only work required to preserve existing API, database, lifecycle, messaging, and C1/C2 call semantics.
