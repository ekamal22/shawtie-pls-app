...
- C1 Voice Calling: DONE on `feat/c1-voice-calling` (unmerged); source implementation and final integrated automated/local closure are complete at `9b5c255b5e8c60cbe8da4bcd2b6f7596c56687a0` against real migrations 0001 through 0018 with `reserved=0`. Mandatory physical Android acceptance then passed 25/25 on the Redmi Note 9S at `9cbc2f8194591e95752eb3a7a9f771e340974b19` (executable code identical to `9b5c255`, no defects found, no code changed, `C1_ANDROID_ACCEPTANCE_PASS`; evidence in `docs/testing/C1_ANDROID_ACCEPTANCE_EVIDENCE.md`). C1 is not merged and awaits independent verification and an explicit merge decision.
...
7. merge C1 only on an explicit merge instruction after independent verification; keep C2 Video Calling separate and not started until then
