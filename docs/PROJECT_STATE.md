...
- UX1 Romantic Design Foundation: DONE on `feat/ux1-romantic-foundation`. Tokens, Midnight and Dawn themes, self-hosted OFL fonts, primitives, and the Home, Talk, Ours shell are implemented over unchanged product semantics; see `docs/design/UX1_FOUNDATION.md`. UX2 through UX7 are the next presentation milestones.
- UX2 through UX6: INTEGRATED on `integration/ux-romantic` (not yet merged to `main`). Home, Talk (including the read-receipt gate to the active Talk view and the Remember This Ribbon), Ours with Then, Now and Next and the Us surface, full-screen call presentation, and the memory, letter and time views are implemented over unchanged product semantics. Cross-surface Chromium smoke covers Memory Return and release-gated privacy. UX7 signature moments, the visual and accessibility reviews, and the Redmi Note 9S sweep remain.
- UX7: PLANNED presentation-only signature moments after the UX2 through UX6 baseline.
