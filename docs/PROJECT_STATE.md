...
S1 E2EE and Cryptographic Recovery is IN_PROGRESS on `feat/s1-e2ee-crypto-recovery`. Source implementation for S1-A through S1-I and the S1-J automated/physical closure tooling are committed at implementation/harness anchor `c85fdb1`. The branch owns migrations 0019 through 0021 and now contains the pinned OpenMLS `0.9.0` / `openmls_rust_crypto 0.6.0` WASM binding, independent device crypto identities and KeyPackages, partnership MLS groups/generations/epochs/control stream, protected M1/M2/R1/M3 flows, client content-signature verification, recovery bundles and HPKE capsules, revocation/rekey and recovery-authorized group reset, plaintext retirement inventory and DB guards, S1 API/browser/integration suites, real Chromium WASM harness, production-bundle scan, disposable local closure runner, and a 30-scenario physical Android acceptance procedure. No S1 closure PASS is claimed yet: `npm run test:s1:closure`, raw PostgreSQL/object-storage/log/outbox/push plaintext inspections, 30/30 physical Android acceptance, and final independent cryptographic/security review still require execution evidence. Until those gates pass, S1 remains IN_PROGRESS and the product must not claim verified E2EE. Physical procedure: `docs/testing/S1_ANDROID_ACCEPTANCE.md`.
...
- OpenMLS `0.9.0` with `openmls_rust_crypto 0.6.0` pinned in the S1 implementation baseline; final dependency/advisory review remains an S1-J gate
...
- S1 E2EE and Cryptographic Recovery: IN_PROGRESS on `feat/s1-e2ee-crypto-recovery`. S1-A through S1-I source implementation plus S1-J closure tooling are committed at `c85fdb1`; migrations 0019 through 0021, OpenMLS WASM runtime, protected M1/M2/R1/M3 flows, recovery, rekey/reset, plaintext retirement guards, automated closure harnesses, and the 30-scenario Android procedure are present. Executed automated closure, raw plaintext inspections, physical Android acceptance, and final independent security review remain open. UX copy must not claim verified E2EE until S1-J closes.
...
- `feat/s1-e2ee-crypto-recovery` -> active S1 implementation branch; source implementation and closure harnesses complete at `c85fdb1`, S1-J execution evidence pending
