...
M1 and R1 remain separate parallel branches. M1 runtime work continues independently. R1 source implementation is complete and now needs executable closure evidence plus any fixes surfaced by those gates.

1. continue M1 only in `feat/m1-messaging-core` with ownership of migrations 0011 and 0012
2. run the R1 closure harness on `feat/r1-relationship-space`, fix only failures surfaced by validation, and preserve R1 ownership of migrations 0013 and 0014
