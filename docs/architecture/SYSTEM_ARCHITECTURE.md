...
C1 does not add Redis, an SFU/MCU, call recording, direct peer fallback, or video. C2 source implementation and automated/local closure are complete at `94e0e93` on the same authority model, with `shawtie.call.v2`, stable video transceivers, generation-fenced local camera control, explicit video UI, shared relay-only transport and no new durable call aggregate or database migration. Physical verification remains pending.
